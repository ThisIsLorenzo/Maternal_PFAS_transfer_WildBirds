...
# it seems VCV matrix changes the results a bit  
# this model should not run but it does due to VCV (if life_stage_P is included)
# this is a bug
...
mod1 <- mod_results(full_model0x, group = "study_ID", mod = "functional_group", weights = "prop")
orchard_plot(mod1, mod = "functional_group", group = "study_ID", xlab = "lnRR", angle = 45, trunk.size = 0.5)

mod2 <- mod_results(full_model0x, group = "study_ID", mod = "diet", weights = "prop")
orchard_plot(mod2, mod = "diet", group = "study_ID", xlab = "lnRR", angle = 45, trunk.size = 0.5)


saveRDS(mod1, file = here("Rdata", "fung_mod.RDS"))

saveRDS(mod2, file = here("Rdata", "diet_mod.RDS"))
#save(full_model0, file = here("Rdata", "full_model0.RData"))


# try <- orchaRd::mod_results(full_model0, group = "study_ID", mod = "functional_group", weights = "prop")
...
mod1 <- mod_results(full_model1, group = "study_ID", mod = "functional_group", weights = "prop")
orchard_plot(mod1, mod = "functional_group", group = "study_ID", xlab = "lnRR", angle = 45, trunk.size = 0.5)


test0 <- rma.mv(yi = lnRR,
                      V = var_lnRR, 
                      mods = ~  1 + 
                        functional_group +
                        scale(carbon_chain_length) +
...
# looking at what is in the life stage - all eggs
...
# for model selection, we should use ML (not the refoult REML) to estimate the model (it may automatically do this)

mod.candidate <- dredge(full_model1, evaluate = TRUE, rank = "AICc", trace = 2) 
