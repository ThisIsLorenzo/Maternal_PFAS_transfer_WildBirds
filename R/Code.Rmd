...
```{r, results = 'hide'}
st <- read_csv(here("Data", "study_info.csv")) #study-related data
# dim(st)
# [1] 13 16
sp <- read_csv(here("Data", "species_info.csv")) #species-related data
# dim(sp)
# [1] 16 15
pfas <- read_csv(here("Data", "pfas_info.csv")) #pfas-related data
# dim(pfas)
# [1] 26 12
co <- read_csv(here("Data", "cohort_info.csv")) #cohort-related data
# dim(co)
# [1] 63 25
sa <- read_csv(here("Data", "sample_info.csv")) #sample-related data
# dim(sa)
# [1] 70  5
me <- read_csv(here("Data", "measurement_info.csv")) #statistics
# dim(me)
# [1] 505  25
```
...
Saving the dat file so we can use it to run model in the Model Outputs associated file.
...
life_stage_model_rob <- robust(life_stage_model, dat$measurement_ID_P)
summary(life_stage_model)
r2_ml(life_stage_model, dat)
   # R2_marginal R2_conditional 
   #  0.00278897     0.07829010 

fig_life_stage_p <- orchard_plot(life_stage_model_rob,
             mod = "life_stage_P",
             group = "study_ID",
             xlab = "lnRR",
             angle = 45,
...
# ggsave(here("figs","AICc_weights.png"),
#        width = 12,
#        height = 8)
```
...
# ggsave(here("figs/Suppl","small_study_bias.png"),
#        width = 9,
#        height = 7)
```
...
# ggsave(here("figs/Suppl","time_lag_bias.png"),
#        width = 9,
#        height = 7)
```
...
#Save the plot
ggsave(here("figs","leaveoneout.png"),
       width = 7,
       height = 5)
```
...
dat2_sa_model <- rma.mv(yi = lnRR,
             ...
             test = "t",
             data = dat2_sa)
dat2_sa_model_rob <- robust(dat2_sa_model, dat2_sa$measurement_ID_P)

save(dat2_sa_model_rob, file = here("Rdata", "dat2_sa_model_rob.RData"))
```

```{r, eval=FALSE}
load(here("RData", "dat2_sa_model_rob.RData"))
summary(dat2_sa_model_rob)
I2 <- orchaRd::i2_ml(dat2_sa_model_rob)
i2_ml(dat2_sa_model_rob)
orchaRd::orchard_plot(dat2_sa_model_rob,
             mod = "1",
             group = "study_ID", 
             xlab = "lnRR",
             alpha = 0.6,
...
# ggsave(here("figs/Suppl","SFigure_8.png"),
#        width = 8,
#        height = 6)
```
...
ma_model_rob2 <- robust(ma_model2, dat$measurement_ID_P) #The robust() function computes robust standard errors that account for non-normality and/or heteroscedasticity in the residuals.

I2 <- orchaRd::i2_ml(ma_model_rob2)
i2_ml(ma_model_rob2)
  #  I2_Total I2_study_ID  I2_pfas_ID    I2_es_ID
  # 99.001034    4.030746    2.329994   92.640294

ma_model_rob_bubble2 <- orchaRd::orchard_plot(ma_model_rob2,
             mod = "1",
             group = "study_ID", 
             xlab = "lnRR",
             alpha = 0.6,
...
ma_model_rob_bubble2
#Save the plot
# ggsave(here("figs","ma_model_rob_LOD.png"),
#        width = 7,
#        height = 5)
...
## Excluding values unde limit of detection/quantification

```{r}
dat3_sa <-
  dat %>%
  dplyr::filter(limit_type_A != "LOQ" |
                  limit_type_A != "LOD" |
                  limit_type_P != "LOQ"|
                  limit_type_P != "LOD"
                  )
...
VCV_lnRR_dat3 <- impute_covariance_matrix(dat3_sa$var_lnRR,
                                          cluster = dat3_sa$measurement_ID_A,
                                          r = 0.5)
```
```{r, eval=FALSE}
dat3_sa_model <- rma.mv(yi = lnRR,
             V = VCV_lnRR_dat3, 
             random = list(~1|study_ID,
                            ~1|pfas_ID,
                            ~1|es_ID
             ...
             test = "t",
             data = dat3_sa)
dat3_sa_model_rob <- robust(dat3_sa_model, dat3_sa$measurement_ID_P)

save(dat3_sa_model_rob, file = here("Rdata", "dat3_sa_model_rob.RData"))
```

```{r, eval=FALSE}
load(here("RData", "dat3_sa_model_rob.RData"))
summary(dat3_sa_model_rob)
I2 <- orchaRd::i2_ml(dat3_sa_model_rob)
i2_ml(dat3_sa_model_rob)
orchaRd::orchard_plot(dat3_sa_model_rob,
             mod = "1",
             group = "study_ID", 
             xlab = "lnRR",
             alpha = 0.6,
